    ...
    def __init__(
        ...
        Args:
            ...
            cycle_barriers (bool, optional): insert a global barrier before
                every circuit cycle during QubiC transpilation. Defaults
                to True (historical behavior). Set False for
                desynchronized execution: only explicit Barrier objects
                synchronize qubits, so each qubit's readout fires right
                after its own gates or delays.
